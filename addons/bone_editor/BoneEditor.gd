...
func save_pose( ):
	...
	self._add_bone_tracks( animation )
	# ポーズを保存
	self._save_pose_to_animation( animation )
...
func _save_pose_to_animation( animation:Animation ):
	var time: float = self.animation_player.current_animation_position
	var selected: Array = edi.get_selection().get_selected_nodes()
	if selected.empty():
